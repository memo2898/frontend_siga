...
import type { Sedes, SedesCreateDTO, SedesUpdateDTO, SedesFilters, PaginatedResponse } from './sedes.types';

const BASE = `${SERVER_ROUTE}/api/sedes`;
...
export const getAll = async () => {
  const response = await http.get<Sedes[]>(BASE);
...
export const getById = async (id: number) => {
  return await http.get<Sedes>(`${BASE}/${id}`);
};

export const create = async (data: SedesCreateDTO) => {
  const payload = {
    ...data,
    estado: 'ACTIVO',
    agregado_en: new Date().toISOString(),
  };
  return await http.post<Sedes>(BASE, payload);
};

export const update = async (id: number, data: SedesUpdateDTO) => {
  const payload = {
    ...data,
    actualizado_en: new Date().toISOString(),
  };
  return await http.patch<Sedes>(`${BASE}/${id}`, payload);
};
...
export const getPaginated = async (filters: SedesFilters = {}) => {
  const query = buildQuery(filters);
  const response = await http.get<PaginatedResponse<Sedes>>(`${BASE}/paginated?${query}`);
