...
import type { Categorias, CategoriasCreateDTO, CategoriasUpdateDTO, CategoriasFilters, PaginatedResponse } from './categorias.types';

const BASE = `${SERVER_ROUTE}/api/categorias`;
...
export const getAll = async () => {
  const response = await http.get<Categorias[]>(BASE);
...
export const getPaginated = async (filters: CategoriasFilters = {}) => {
  const query = buildQuery(filters);
  const response = await http.get<PaginatedResponse<Categorias>>(`${BASE}/paginated?${query}`);
