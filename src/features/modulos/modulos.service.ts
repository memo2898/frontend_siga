...
import type { Modulos, ModulosCreateDTO, ModulosUpdateDTO, ModulosFilters, PaginatedResponse } from './modulos.types';

const BASE = `${SERVER_ROUTE}/api/modulos`;
...
export const getAll = async () => {
  const response = await http.get<Modulos[]>(BASE);
...
export const getById = async (id: number) => {
  return await http.get<Modulos>(`${BASE}/${id}`);
};

export const create = async (data: ModulosCreateDTO) => {
  const payload = {
    ...data,
    estado: 'ACTIVO',
    agregado_en: new Date().toISOString(),
  };
  return await http.post<Modulos>(BASE, payload);
};

export const update = async (id: number, data: ModulosUpdateDTO) => {
  const payload = {
    ...data,
    actualizado_en: new Date().toISOString(),
  };
  return await http.patch<Modulos>(`${BASE}/${id}`, payload);
};
...
export const getPaginated = async (filters: ModulosFilters = {}) => {
  const query = buildQuery(filters);
  const response = await http.get<PaginatedResponse<Modulos>>(`${BASE}/paginated?${query}`);
