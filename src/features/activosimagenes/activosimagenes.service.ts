...
import type { ActivosImagenes, ActivosImagenesCreateDTO, ActivosImagenesUpdateDTO, ActivosImagenesFilters, PaginatedResponse } from './activosimagenes.types';

const BASE = `${SERVER_ROUTE}/api/activos-imagenes`;
...
export const getAll = async () => {
  const response = await http.get<ActivosImagenes[]>(BASE);
...
export const getById = async (id: number) => {
  return await http.get<ActivosImagenes>(`${BASE}/${id}`);
};

export const create = async (data: ActivosImagenesCreateDTO) => {
  const payload = {
    ...data,
    estado: 'ACTIVO',
    agregado_en: new Date().toISOString(),
  };
  return await http.post<ActivosImagenes>(BASE, payload);
};

export const update = async (id: number, data: ActivosImagenesUpdateDTO) => {
  const payload = {
    ...data,
    actualizado_en: new Date().toISOString(),
  };
  return await http.patch<ActivosImagenes>(`${BASE}/${id}`, payload);
};
...
export const getPaginated = async (filters: ActivosImagenesFilters = {}) => {
  const query = buildQuery(filters);
  const response = await http.get<PaginatedResponse<ActivosImagenes>>(`${BASE}/paginated?${query}`);
