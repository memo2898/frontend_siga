...
import type { TemplatesModulos, TemplatesModulosCreateDTO, TemplatesModulosUpdateDTO, TemplatesModulosFilters, PaginatedResponse } from './templatesmodulos.types';

const BASE = `${SERVER_ROUTE}/api/templates-modulos`;
...
export const getAll = async () => {
  const response = await http.get<TemplatesModulos[]>(BASE);
...
export const getById = async (id: number) => {
  return await http.get<TemplatesModulos>(`${BASE}/${id}`);
};

export const create = async (data: TemplatesModulosCreateDTO) => {
  const payload = {
    ...data,
    estado: 'ACTIVO',
    agregado_en: new Date().toISOString(),
  };
  return await http.post<TemplatesModulos>(BASE, payload);
};

export const update = async (id: number, data: TemplatesModulosUpdateDTO) => {
  const payload = {
    ...data,
    actualizado_en: new Date().toISOString(),
  };
  return await http.patch<TemplatesModulos>(`${BASE}/${id}`, payload);
};
...
export const getPaginated = async (filters: TemplatesModulosFilters = {}) => {
  const query = buildQuery(filters);
  const response = await http.get<PaginatedResponse<TemplatesModulos>>(`${BASE}/paginated?${query}`);
