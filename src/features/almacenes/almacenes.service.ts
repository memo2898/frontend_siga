...
export const getAll = async () => {
  const response = await http.get<Almacenes[]>(BASE);
...
export const getById = async (id: number) => {
  return await http.get<Almacenes>(`${BASE}/${id}`);
};

export const create = async (data: AlmacenesCreateDTO) => {
  const payload = {
    ...data,
    estado: 'ACTIVO',
    agregado_en: new Date().toISOString(),
  };
  return await http.post<Almacenes>(BASE, payload);
};

export const update = async (id: number, data: AlmacenesUpdateDTO) => {
  const payload = {
    ...data,
    actualizado_en: new Date().toISOString(),
  };
  return await http.patch<Almacenes>(`${BASE}/${id}`, payload);
};
