...
import type { ModulosCategorias, ModulosCategoriasCreateDTO, ModulosCategoriasUpdateDTO, ModulosCategoriasFilters, PaginatedResponse } from './moduloscategorias.types';

const BASE = `${SERVER_ROUTE}/api/modulos-categorias`;
...
export const getAll = async () => {
  const response = await http.get<ModulosCategorias[]>(BASE);
...
export const getById = async (id: number) => {
  return await http.get<ModulosCategorias>(`${BASE}/${id}`);
};

export const create = async (data: ModulosCategoriasCreateDTO) => {
  const payload = {
    ...data,
    estado: 'ACTIVO',
    agregado_en: new Date().toISOString(),
  };
  return await http.post<ModulosCategorias>(BASE, payload);
};

export const update = async (id: number, data: ModulosCategoriasUpdateDTO) => {
  const payload = {
    ...data,
    actualizado_en: new Date().toISOString(),
  };
  return await http.patch<ModulosCategorias>(`${BASE}/${id}`, payload);
};
...
export const getPaginated = async (filters: ModulosCategoriasFilters = {}) => {
  const query = buildQuery(filters);
  const response = await http.get<PaginatedResponse<ModulosCategorias>>(`${BASE}/paginated?${query}`);
