import type { Templates, TemplatesGridRow } from '../templates.types';

export interface TemplatesHandlers {
  onEdit: (templates: Templates) => void;
  onDelete: (templates: Templates) => void;
}
...
export const toTemplatesGridRow = (templates: Templates, handlers: TemplatesHandlers): TemplatesGridRow => ({
  ...
  actualizado_en: templates.actualizado_en,
  estado: templates.estado,
  actions: [
    {
      content: `
        ${actionButtonStyles}
      ...
      `,
      event: 'click',
      funct: () => handlers.onEdit(templates),
    },
    {
      content: `
      ...
      `,
      event: 'click',
      funct: () => handlers.onDelete(templates),
    },
  ],
});

export const toTemplatesGridRows = (templates: Templates[], handlers: TemplatesHandlers): TemplatesGridRow[] =>
  templates.map((templates) => toTemplatesGridRow(templates, handlers));
