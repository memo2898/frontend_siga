import { ModalX } from '../../../lib/uiX/components/ModalX';
import type { Activos } from '../activos.types';

interface ActivosDeleteModalProps {
  isOpen: boolean;
  activos: Activos | null;
  loading?: boolean;
  onConfirm: () => void;
  onCancel: () => void;
}

export function ActivosDeleteModal({ isOpen, activos, loading, onConfirm, onCancel }: ActivosDeleteModalProps) {
  if (!activos) return null;

  return (
    <ModalX
      isOpen={isOpen}
      onClose={onCancel}
      title="Eliminar Activos"
      size="sm"
      footer={
        <>
          <button onClick={onCancel} disabled={loading} className="btn btn-secondary">
            Cancelar
          </button>
          <button onClick={onConfirm} disabled={loading} className="btn btn-danger">
            {loading ? 'Eliminando...' : 'Eliminar'}
          </button>
        </>
      }
    >
      <p style={ { marginBottom: 16 } }>
        ¿Estás seguro que deseas eliminar el siguiente activos?
      </p>
      <div style={ {
        padding: 16,
        backgroundColor: '#fef2f2',
        borderRadius: 8,
        border: '1px solid #fecaca',
      } }>
        <strong>{ activos.codigo_inventario_local }</strong>
        <br />
        <span style={ { color: '#6b7280', fontSize: 14 } }>
          { activos.codigo_inventario_control_bienes }
        </span>
      </div>
      <p style={ { marginTop: 16, color: '#ef4444', fontSize: 14 } }>
        Esta acción no se puede deshacer.
      </p>
    </ModalX>
  );
}
