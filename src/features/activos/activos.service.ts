...
import type { Activos, ActivosCreateDTO, ActivosUpdateDTO, ActivosFilters, PaginatedResponse } from './activos.types';

const BASE = `${SERVER_ROUTE}/api/activos`;
...
export const getAll = async () => {
  const response = await http.get<Activos[]>(BASE);
...
export const getById = async (id: number) => {
  return await http.get<Activos>(`${BASE}/${id}`);
};

export const create = async (data: ActivosCreateDTO) => {
  const payload = {
    ...data,
    estado: 'ACTIVO',
    agregado_en: new Date().toISOString(),
  };
  return await http.post<Activos>(BASE, payload);
};

export const update = async (id: number, data: ActivosUpdateDTO) => {
  const payload = {
    ...data,
    actualizado_en: new Date().toISOString(),
  };
  return await http.patch<Activos>(`${BASE}/${id}`, payload);
};
...
export const getPaginated = async (filters: ActivosFilters = {}) => {
  const query = buildQuery(filters);
  const response = await http.get<PaginatedResponse<Activos>>(`${BASE}/paginated?${query}`);
