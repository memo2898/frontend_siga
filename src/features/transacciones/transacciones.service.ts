...
import type { Transacciones, TransaccionesCreateDTO, TransaccionesUpdateDTO, TransaccionesFilters, PaginatedResponse } from './transacciones.types';

const BASE = `${SERVER_ROUTE}/api/transacciones`;
...
export const getAll = async () => {
  const response = await http.get<Transacciones[]>(BASE);
...
export const getById = async (id: number) => {
  return await http.get<Transacciones>(`${BASE}/${id}`);
};

export const create = async (data: TransaccionesCreateDTO) => {
  const payload = {
    ...data,
    estado: 'ACTIVO',
    agregado_en: new Date().toISOString(),
  };
  return await http.post<Transacciones>(BASE, payload);
};

export const update = async (id: number, data: TransaccionesUpdateDTO) => {
  const payload = {
    ...data,
    actualizado_en: new Date().toISOString(),
  };
  return await http.patch<Transacciones>(`${BASE}/${id}`, payload);
};
...
export const getPaginated = async (filters: TransaccionesFilters = {}) => {
  const query = buildQuery(filters);
  const response = await http.get<PaginatedResponse<Transacciones>>(`${BASE}/paginated?${query}`);
